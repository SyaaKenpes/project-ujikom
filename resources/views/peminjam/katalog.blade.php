...
<div class="bg-white p-5 rounded-xl shadow-sm border border-gray-200">
    <form action="{{ route('peminjam.katalog.index') }}" method="GET" class="flex flex-col md:flex-row items-end gap-4">
        <div class="w-full md:w-1/3">
            ...
            <select name="kategori_id" id="kategori_id" onchange="this.form.submit()" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition cursor-pointer">
    ...
    <form action="{{ route('peminjam.peminjaman.store') }}" method="POST">
        ...
        <div class="bg-white p-5 rounded-xl shadow-sm border border-gray-200 flex flex-col md:flex-row items-center justify-between gap-4">
            <div class="w-full md:w-1/2">
                ...
                <input type="date" name="tgl_kembali_plan" id="tgl_kembali_plan" 
                    min="{{ date('Y-m-d') }}" required
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition">
            </div>
            <div class="w-full md:w-auto flex justify-end">
                <button type="submit" class="w-full md:w-auto bg-blue-600 hover:bg-blue-700 text-white font-semibold px-6 py-2.5 rounded-lg shadow transition flex items-center justify-center gap-2">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 mt-6">
            @forelse($alats as $alat)
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col justify-between p-4 hover:shadow-md transition">
                    <div>
                        <!-- AREA FOTO BARANG -->
                        <div class="w-full h-40 bg-gray-100 rounded-lg mb-3 overflow-hidden border border-gray-200 flex items-center justify-center">
                            @if($alat->gambar) 
                        ...
                        <div class="flex items-center justify-between mb-2">
                            ...
                            @php
                                ...
                                $color = $kondisi == 'baik' ? 'bg-emerald-100 text-emerald-700' : 
                                        ($kondisi == 'rusak' ? 'bg-red-100 text-red-700' : 'bg-blue-50 text-blue-600');
                            @endphp
                    ...
                    <div class="pt-4 border-t border-gray-100 space-y-3">
                        ...
                        <div>
                            ...
                            <input type="number" name="jumlah[]" id="jumlah_{{ $alat->id }}" value="1" min="1" max="{{ $alat->stok }}" disabled required
